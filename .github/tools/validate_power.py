...
# upstream strings. The remaining binary assets (PNG, the vendored d3 bundle) are
# hashed by the manifest check but never read as text.
TEXT_SUFFIXES = {".md", ".py", ".json"}

# PDFs are scanned for residual upstream strings too, via the text extractor
# below. They are excluded from TEXT_SUFFIXES rather than added to it because a
# PDF has no lines to report and no path references to resolve: only the
# residual scan applies, and it locates a hit by content stream instead.
#
# This is not hypothetical tidiness. The shipped example recap PDF carried
# "Generated by the Senzing Bootcamp Claude plugin" and a certificate colophon
# reading "Senzing Bootcamp Claude plugin v0.5.0" long after the generator that
# produces it had been corrected, because the pre-built asset was never
# regenerated. The manifest check could not catch it: a SHA-256 confirms the
# bytes match what was recorded, not that the recorded bytes were ever right.
PDF_SUFFIXES = {".pdf"}
...
def _decode_pdf_literal(raw: bytes) -> str:
    """Resolve the escapes in a PDF ``(...)`` string literal."""
    out = bytearray()
    index = 0
    while index < len(raw):
        byte = raw[index]
        ...
        if escape in simple:  # \n \r \t \b \f
            ...
            index += 1
        elif 0x30 <= escape <= 0x37:  # octal, up to three digits
            digits = bytearray()
            while index < len(raw) and len(digits) < 3 and 0x30 <= raw[index] <= 0x37:
                digits.append(raw[index])
                index += 1
            out.append(int(digits.decode("ascii"), 8) & 0xFF)
        elif escape == 0x0A:  # line continuation: the newline is not content
            index += 1
        else:  # \( \) \\ and any other escaped literal
            out.append(escape)
            index += 1
    return out.decode("latin-1")


# A ``(...)`` string literal, honouring backslash escapes so that an escaped
# closing paren does not end the match early.
PDF_STRING_LITERAL = re.compile(rb"\((?:[^()\\]|\\.)*\)", re.S)
PDF_HEX_LITERAL = re.compile(rb"<([0-9A-Fa-f\s]+)>")
PDF_STREAM = re.compile(rb"stream\r?\n(.*?)endstream", re.S)


def pdf_text_runs(path: Path) -> list[str]:
    """The visible text of a PDF, one entry per content stream, stdlib only.

    Returns two forms of every stream's text so that a needle survives kerning.
    A PDF writer is free to split a word across adjacent string literals, as
    ``[(Cla)-12(ude)] TJ``, so joining literals on a space would hide
    "Claude" from a substring scan. Each stream therefore contributes both the
    space-joined form (correct word boundaries) and the tightly-joined form
    (survives a split word), and the caller scans both.

    Deliberately not a general PDF parser: it reads FlateDecode and plain
    content streams, which is what the Power's own generator emits. A stream it
    cannot decode contributes nothing, and ``check_residual_upstream`` fails a
    PDF that yields no text at all rather than passing it.
    """
    raw = path.read_bytes()
    runs: list[str] = []

    for match in PDF_STREAM.finditer(raw):
        body = match.group(1)
        try:
            body = zlib.decompress(body)
        except zlib.error:
            # Not Flate-compressed: it may be a plain content stream, or an image
            # whose bytes are simply not text. Scanning it raw is harmless.
            pass

        literals = [_decode_pdf_literal(m.group()[1:-1]) for m in PDF_STRING_LITERAL.finditer(body)]
        for hexed in PDF_HEX_LITERAL.finditer(body):
            digits = re.sub(rb"\s", b"", hexed.group(1))
            if len(digits) % 2:
                digits += b"0"  # an odd-length hex string is padded per the spec
            try:
                literals.append(bytes.fromhex(digits.decode("ascii")).decode("latin-1"))
            except ValueError:
                continue

        if not literals:
            continue
        runs.append(" ".join(literals))
        runs.append("".join(literals))
...
def check_residual_upstream(root: Path) -> Check:
    ...
    for relative in shipped_files(root):
        ...
        if is_pdf:
            ...
            if not runs:
                # Failing open here would rebuild the blind spot this scan exists
                # to close, so an unreadable PDF is a finding, not a pass.
                check.findings.append(
                    f"{key}: no text could be extracted, so the residual scan "
                    "could not run on a shipped PDF"
                )
                continue
            seen: set[str] = set()
